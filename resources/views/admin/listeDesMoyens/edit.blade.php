...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.listeDesMoyen.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.liste-des-moyens.update", [$listeDesMoyen->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('note') ? 'is-invalid' : '' }}" name="note" id="note">{!! old('note', $listeDesMoyen->note) !!}</textarea>
                @if($errors->has(''))
                    <span class="text-danger">{{ $errors->first('') }}</span>
                @endif
